 ...
 * - Skeletal body-signature matching when face is out of view (e.g. glute bridge),
 *   using the body signature enrolled at initial verify time
 ...
 * [TrustState.TRUSTED] is restored. This monitor never owns or resets rep/time
 * progress — callers must preserve session counters across pause/resume.
 */
class ContinuousIdentityMonitor(
    private val verifyFace: (probe: FloatArray, enrolled: FloatArray, threshold: Float) -> Pair<Boolean, Float>,
    ...
    constructor(
        ...
        faceMatchThreshold: Float = FaceRecognitionService.MATCH_THRESHOLD
    ) : this(
        verifyFace = { probe, enrolled, threshold ->
            faceService.verifyAgainstEnrollment(probe, enrolled, threshold)
        },
    ...
    private val skeletonSession = SkeletonPresenceTracker.Session(nowMs)
    ...
    fun onFrame(
        ...
        if (faceEmbedding != null && enrolledFace != null) {
            val (matched, similarity) = verifyFace(
                faceEmbedding,
                enrolledFace,
                faceMatchThreshold
            )
