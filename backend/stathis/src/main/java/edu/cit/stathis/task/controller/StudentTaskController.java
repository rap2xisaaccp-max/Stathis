...
public class StudentTaskController {
    ...
    @Operation(summary = "Mark exercise as completed", description = "Saves reps/accuracy/calories, accumulates totals on repeat attempts, and syncs progress to teachers")
    public ResponseEntity<Score> completeExercise(
            @PathVariable String taskId,
            @PathVariable String exerciseTemplateId,
            @RequestBody(required = false) ExerciseResultSubmissionDTO result) {
        String studentId = physicalIdService.getCurrentUserPhysicalId();
        return ResponseEntity.ok(
                studentTaskService.completeExercise(studentId, taskId, exerciseTemplateId, result));
    }
} 